...
export const BillingRouter = Trpc.createRouter({
  ...
  findManyPayments: Trpc.procedure.query(async ({ ctx }) => {
    ...
    const user = await ctx.database.user.findFirst({ where: { id: userId } })
  ...
  getPaymentLink: Trpc.procedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const userId = ctx.session?.user?.id
      let user = await ctx.database.user.findFirst({ where: { id: userId } })
      ...
      const urlRedirection = process.env.BASE_URL
